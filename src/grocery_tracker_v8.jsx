...
import { loadGsiScript, signIn, silentSignIn, signOut, GOOGLE_CLIENT_ID } from "./lib/auth";
import {
...
const FALLBACK_ITEMS = [
  { category: "Dairy & Eggs", name: "Eggs",                  size: "18 ct",  prices: { winco: 2.92, fredmeyer: null, safeway: null, yokes: null, costco: null }, onSale: {} },
  { category: "Dairy & Eggs", name: "Darigold Butter",       size: "1 lb",   prices: { winco: 8.61, fredmeyer: null, safeway: null, yokes: null, costco: null }, onSale: {} },
  { category: "Dairy & Eggs", name: "Plain Yogurt",          size: "48 oz",  prices: { winco: 5.95, fredmeyer: null, safeway: null, yokes: null, costco: null }, onSale: {} },
  { category: "Dairy & Eggs", name: "Heavy Cream",           size: "32 oz",  prices: { winco: 5.11, fredmeyer: null, safeway: null, yokes: null, costco: null }, onSale: {} },
  { category: "Produce",      name: "Bananas",               size: "1 lb",   prices: { winco: 0.50, fredmeyer: null, safeway: null, yokes: null, costco: null }, onSale: {} },
  { category: "Produce",      name: "Avocado",               size: "each",   prices: { winco: 0.68, fredmeyer: null, safeway: null, yokes: null, costco: null }, onSale: {} },
  { category: "Produce",      name: "Roma Tomatoes",         size: "1 lb",   prices: { winco: 0.92, fredmeyer: null, safeway: null, yokes: null, costco: null }, onSale: {} },
  { category: "Produce",      name: "Potatoes (Bakers)",     size: "1 lb",   prices: { winco: 0.48, fredmeyer: null, safeway: null, yokes: null, costco: null }, onSale: {} },
  { category: "Produce",      name: "Onions, Yellow",        size: "1 lb",   prices: { winco: 0.68, fredmeyer: null, safeway: null, yokes: null, costco: null }, onSale: {} },
  { category: "Produce",      name: "Sweet Mini Peppers",    size: "16 oz",  prices: { winco: 2.98, fredmeyer: null, safeway: null, yokes: null, costco: null }, onSale: {} },
  { category: "Produce",      name: "Spring Mix",            size: "24 oz",  prices: { winco: 3.98, fredmeyer: null, safeway: null, yokes: null, costco: null }, onSale: {} },
  { category: "Pantry",       name: "Green Chili",           size: "4 oz",   prices: { winco: 0.88, fredmeyer: null, safeway: null, yokes: null, costco: null }, onSale: {} },
  { category: "Pantry",       name: "Pillsbury Croissants",  size: "1 tube", prices: { winco: 2.58, fredmeyer: null, safeway: null, yokes: null, costco: null }, onSale: {} },
  { category: "Beverages",    name: "Yellowtail Sauv Blanc", size: "750 ml", prices: { winco: 5.41, fredmeyer: null, safeway: null, yokes: null, costco: null }, onSale: {} },
];
...
const STORES = [
  ...
  { key: "costco",    label: "Costco",     short: "Costco", color: "#005DAA", bg: "#EEF3FC" },
];
...
const AD_LINKS = {
  ...
  costco:    "https://www.costco.com/weekly-savings",
};
  ...
  return (scriptItems || [])
    ...
    .map((item, idx) => ({
      ...
      prices: {
        ...
        costco:    item.prices?.costco    ?? null,
      },
      onSale: { winco: false, fredmeyer: false, safeway: false, yokes: false, costco: false },
    }));
}
...
// ── Add Item Modal ────────────────────────────────────────────────────────────
function AddItemModal({ onClose, onAdd, adding, liveCategories }) {
  ...
  const activeCategory = newCatMode ? newCatName.trim() : category;

  const handleSubmit = () => {
    if (!activeCategory || !name.trim() || !size.trim()) return;
    onAdd(activeCategory, name.trim(), size.trim());
  };
  ...
  return (
    <div style={{
      ...
      <div style={{
        background: "#1E1E1E", width: "100%", maxWidth: 560, margin: "0 auto",
        ...
        <div style={{ marginBottom: 10 }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 3 }}>
            <label style={{ fontSize: 10, color: "#888", textTransform: "uppercase", letterSpacing: 0.5 }}>Category</label>
            <button
              onClick={() => { setNewCatMode(m => !m); setNewCatName(""); }}
              style={{ fontSize: 9, background: "none", border: "none", color: newCatMode ? "#F87171" : "#1B6B35", cursor: "pointer", fontWeight: 700, padding: 0 }}
            >
              {newCatMode ? "✕ Cancel new" : "+ New category"}
            </button>
          </div>
          ...
          {newCatMode ? (
            <input
              ...
              autoFocus
              style={{ width: "100%", padding: "7px 8px", borderRadius: 6, border: "1px solid #1B6B35", fontSize: 13, background: "#2A2A2A", color: "#F5F0E8", boxSizing: "border-box" }}
            />
          ) : (
            <select
              value={category}
            ...
            >
              {liveCategories.map(c => <option key={c} value={c}>{CAT_ICONS[c] || "📦"} {c}</option>)}
            </select>
          )}
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 2fr", gap: 8, marginBottom: 10 }}>
          ...
          <div>
            ...
            <input
              ...
              style={{ width: "100%", padding: "7px 8px", borderRadius: 6, border: "1px solid #444", fontSize: 13, background: "#2A2A2A", color: "#F5F0E8", boxSizing: "border-box" }}
            />
          </div>
        </div>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          ...
          <button
            ...
            disabled={adding || !activeCategory || !name.trim() || !size.trim()}
            style={{
          ...
          >
            {adding ? "Adding…" : `Add to ${activeCategory || "…"}`}
          </button>
        </div>
      </div>
    </div>
...
export default function GroceryTracker() {
  ...
  const [categories,    setCategories]    = useState(["Dairy & Eggs", "Produce", "Pantry", "Beverages"]);
  ...
  const loadData = useCallback(async (token) => {
    ...
    try {
      ...
      if (data.length > 0) {
        ...
        // Derive ordered, deduplicated category list from live sheet data
        const liveCats = [...new Map(data.map(i => [i.category, i.category])).keys()]
  ...
  const handleSignOut = () => {
    signOut(user?.accessToken);  // also clears localStorage hint
    setUser(null);
  ...
  // ── RENDER ────────────────────────────────────────────────────────────────
  return (
    <div style={{ fontFamily: "'Georgia', serif", background: "#F8F4ED", minHeight: "100vh", maxWidth: 560, margin: "0 auto", paddingBottom: 48 }}>

      {showAddModal && <AddItemModal onClose={() => setShowAddModal(false)} onAdd={handleAddItem} adding={adding} liveCategories={categories} />}
                  ...
                  {!isCollapsed && catItems.map(item => {
                    ...
                    return (
                      <div key={item.rowNum ?? item.name} style={{
                        ...
                        {/* Price grid — 5-column for 5 stores */}
                        <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 1, background: "#EDE8DF" }}>
                          {STORES.map(store => (
                            <PriceCell
                              key={store.key}
                              item={item}
